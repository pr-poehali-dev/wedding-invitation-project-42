...
// Сентябрь 2026: 1-е число — вторник (индекс 1 при ПН=0)
...
const SCHEDULE = [
  ...
  { time: "16:00", title: "Торжественная церемония", desc: "Вы станете свидетелями создания нашей семьи" },
...
export default function Index() {
  ...
  return (
    <div className="min-h-screen font-cormorant" style={{ background: "#f0ebe3" }}>
          ...
          <div className="relative z-10 flex flex-col items-center justify-between h-full w-full max-w-sm mx-auto px-8 pt-24 pb-24">
            ...
            <div className="animate-fade-in delay-400">
              <svg viewBox="0 0 200 190" width="170" height="160" fill="none">
                ...
                <path d="M100 170 C60 140 10 110 10 65 C10 35 35 15 65 20 C80 23 92 32 100 45 C108 32 120 23 135 20 C165 15 190 35 190 65 C190 110 140 140 100 170Z" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5" fill="none" />
                {[...Array(16)].map((_, i) => (
                  <ellipse key={i} cx="100" cy="92" rx={8 + i * 6} ry={6 + i * 5} stroke="rgba(255,255,255,0.45)" strokeWidth="1" fill="none" clipPath="url(#heart-clip)" />
                ))}
              </svg>
            </div>
      ...
      {/* ОСНОВНОЙ КОНТЕНТ */}
      <div className="transition-opacity duration-1000" style={{ opacity: visible ? 1 : 0 }}>
        ...
        <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden"
          ...
          <div className="relative z-10 text-center px-8 pt-20 pb-16 w-full max-w-sm mx-auto">
            <p className="font-montserrat text-[10px] tracking-[0.4em] uppercase text-[#7a6450] mb-3 animate-fade-up delay-100">
              ПРИГЛАШЕНИЕ
            </p>
            <p className="font-cormorant italic text-[20px] text-[#9e8060] tracking-widest mb-10 animate-fade-up delay-200">
              на свадьбу
            </p>
            ...
            <div className="mt-8">
              <div className="grid grid-cols-7 gap-0 text-center">
                {["ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС"].map(d => (
                  <div key={d} className="font-montserrat text-[11px] tracking-wider text-[#9e8060] py-2">
                    {d}
                  </div>
                ))}
                {CALENDAR_DAYS.map((day, i) => (
                  <div key={i} className="py-1 flex items-center justify-center">
                    {day ? (
                      <span className={`font-cormorant text-[18px] w-9 h-9 flex items-center justify-center relative ${day === 5 ? "text-[#2c1e12]" : "text-[#5a4535]"}`}>
                        {day === 5 && (
          ...
          <div className="flex justify-center gap-3">
            {[
              ...
              <div key={label} className="flex flex-col items-center">
                ...
                <span className="font-montserrat text-[10px] text-[#9e8060] tracking-wider mt-2 uppercase">
                  {label}
                </span>
              </div>
            ))}
              ...
              {SCHEDULE.map((item, i) => (
                <div key={i} className="flex gap-6 items-start">
                  <div className="flex-shrink-0 min-w-[70px]">
                    <p className="font-cormorant font-light text-[28px] text-[#2c1e12] leading-none">
                      {item.time}
                    </p>
                    <div className="w-10 h-px bg-[#c9a96e]/60 mt-2" />
                  </div>
                  <div>
                    <p className="font-cormorant italic text-[22px] text-[#2c1e12] leading-tight mb-1">
                      {item.title}
                    </p>
                    <p className="font-cormorant font-light text-[15px] text-[#7a6450] leading-snug">
                      {item.desc}
                    </p>
                  </div>
                </div>
            ...
            <div className="rounded-sm overflow-hidden mb-6 aspect-[4/3] bg-[#d4c5a9] flex items-center justify-center">
              <img
                ...
                onError={(e) => {
                  ...
                  el.parentElement!.innerHTML = `<div class="w-full h-full flex items-center justify-center"><p class="font-cormorant italic text-[#7a6450] text-xl">Огни Баку</p></div>`;
                }}
              />
            </div>
            <div className="text-center">
              <p className="font-cormorant italic text-[28px] text-[#2c1e12] mb-1">
                Ресторан «Огни Баку»
              </p>
        ...
        <section className="px-8 py-16 max-w-sm mx-auto text-center" style={{ background: "#f0ebe3" }}>
          ...
          <div className="border border-[#c9a96e]/40 p-8 relative">
            ...
            <p className="font-cormorant italic text-[24px] text-[#2c1e12] mb-3">
              Вечерний наряд
            </p>
